...
export default function AppShell({ title, children }) {
  ...
  const navItems = NAV.filter((item) =>
    roleAllowed.some((prefix) => item.href === prefix || item.href.startsWith(`${prefix}/`)),
  ).filter(
    (item, idx, all) => idx === all.findIndex((candidate) => candidate.href === item.href),
  )
    ...
    <div className="hcShell">
      <aside className={cx("hcSidebar", open ? "hcSidebar--open" : "hcSidebar--closed")} aria-label="Primary">
        ...
        <div className="hcSidebar__bottom">
          {open ? <div className="hcSidebar__hint">Tip: you can collapse this sidebar any time.</div> : null}
        </div>
      </aside>
